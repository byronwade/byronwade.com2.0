...
export const softwareDetection = async (content, headers) => {
  try {
    const detectedSoftware: DetectedSoftware = {};

    // Define checks for different frameworks
    const frameworkChecks: FrameworkCheck[] = [
      {
        name: 'React',
        checks: [
          {
      ...
      },
      {
        ...
        checks: [
          ...
          // Check for Tailwind CSS stylesheet link
          {
            ...
            pattern:
              /<link[^>]+href=["'][^"']*\/_next\/static\/[^"']*\/css\/[^"']*\.css["'][^>]*\/>/
          },
    ...
    if (Object.keys(detectedSoftware).length === 0) {
      detectedSoftware.noneDetected =
        'No recognizable software frameworks or libraries were detected based on the predefined checks. This may be due to a custom or less common framework, minified/obfuscated code, or the absence of certain identifiable patterns associated with the frameworks being checked.';
    }
    ...
    for (const check of frameworkChecks) {
      if (check.checks) {
        ...
        for (const subCheck of check.checks) {
          if (subCheck.header) {
            const poweredByHeader = headers.find(
              ([key]) => key.toLowerCase() === subCheck.header
            )?.[1];
            if (poweredByHeader && poweredByHeader.match(subCheck.pattern)) {
              detectedSoftware[check.name.toLowerCase()] = true;
