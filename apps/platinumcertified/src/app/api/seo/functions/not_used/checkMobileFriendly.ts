...
export const checkMobileFriendly = async (page: Page): Promise<Record<string, any>> => {
...
async function isPageMobileFriendly(page: Page): Promise<boolean> {
  try {
    // Check if the page is still open
    if (!page.isClosed()) {
      const hasViewportMeta = await page.$eval('meta[name="viewport"]', (viewportMeta) => {
      ...
      const buttonSizeCheck = await page.$$eval('button', (buttons) => {
