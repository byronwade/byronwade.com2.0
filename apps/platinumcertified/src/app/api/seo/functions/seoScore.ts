...
export const onPageSEOScore = async (domain, content, page) => {
  ...
  if (Array.isArray(keywordAnalysisResult)) {
    ...
    score.keywordDensity =
      (totalWordCount > 0 ? keywordAnalysisResult[0][1] / totalWordCount : 0) * 100;
  }
  ...
  // Page Speed Score (scale from 0 to 10, based on page speed score from 0 to 100)
  const pageSpeedResult = await pageSpeedScore(page);
  score.pageSpeed = parseFloat(pageSpeedResult.pageSpeedScore as string) / 10;
  ...
  score.metaTitleDescription =
    (metadataResult.title.exists ? 5 : 0) + (metadataResult.description.exists ? 5 : 0);
  ...
  score.totalScore =
    ((score.keywordDensity +
      score.mobileFriendly +
      score.pageSpeed +
      score.secureConnection +
      score.metaTitleDescription +
      score.custom404) /
      60) *
    100;
