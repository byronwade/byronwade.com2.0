...
async function mergeAllMushroomVersions() {
	...
	const { data: allMushrooms, error: mushroomsError } = await supabase.from("mushrooms").select("*");
	...
	for (const mushroom of allMushrooms) {
		...
		const { data: olderVersions, error: versionsError } = await supabase.from("mushroom_versions").select("data").eq("mushroom_id", mushroomId).order("version_number", { ascending: false });

		if (versionsError) {
			console.error(`Error fetching older versions for mushroom with ID ${mushroomId}:`, versionsError);
		...
		for (const olderVersion of olderVersions) {
			...
			if (olderData && olderData.openAIResponse && olderData.openAIResponse.common_name) {
				...
				const { error: updateError } = await supabase.from("mushrooms").update({ common_name: mushroom.common_name }).eq("id", mushroomId);
				...
				console.log(`Merged common_name for mushroom with ID ${mushroomId} from version ${olderVersion.version_number}`);
			}
		}
	}
